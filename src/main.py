...
def home():
    if request.method == 'POST':
        ...
        if file:
            ...
            file.save(filename)
            #TODO вставь вызов модели к файлу filename, вывод сохрани в os.path.join(app.config['DOWNLOAD_FOLDER'], "out.csv"))
            os.remove(filename)
...
def download():
    return send_from_directory(app.config['DOWNLOAD_FOLDER'], os.path.join(app.config['DOWNLOAD_FOLDER'], "out.csv"), as_attachment=True)
